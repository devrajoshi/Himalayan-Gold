    ...
    return (
        <div className="space-y-12">
            <header>
                ...
                <p className="text-muted-foreground font-medium">Manage your shipping and billing destinations.</p>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <button className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-primary/40 rounded-2xl bg-background/50 hover:bg-background hover:border-primary transition-all group min-h-[280px]">
                    ...
                    <span className="font-display text-xl font-bold text-forest dark:text-white">Add New Address</span>
                </button>

                {/* Address Cards */}
                {addresses.map((address) => (
                    <div key={address.id} className="bg-background border border-primary/30 p-8 rounded-2xl shadow-sm relative flex flex-col justify-between">
                        <div>
                            <div className="flex justify-between items-start mb-6">
                                <h3 className="font-display text-xl font-bold text-forest dark:text-white">{address.name}</h3>
                                {address.isDefault && (
                                    <span className="bg-primary text-forest text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest">Default</span>
                                )}
                            </div>
            ...
            <section className="bg-background rounded-3xl p-10 shadow-sm border border-muted max-w-3xl">
                <h2 className="font-display text-2xl font-bold text-forest dark:text-white mb-8">Add a New Destination</h2>
                <form className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="col-span-full">
                            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Street Address</label>
                            <input
                        ...
                        <div className="col-span-full">
                            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Apartment/Suite (Optional)</label>
                            <input
                                className="w-full bg-background border-muted rounded-xl focus:ring-1 focus:ring-primary focus:border-primary p-4 outline-none transition-all placeholder:text-muted-foreground/50"
                        ...
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Postal Code</label>
                            <input
                                className="w-full bg-background border-muted rounded-xl focus:ring-1 focus:ring-primary focus:border-primary p-4 outline-none transition-all placeholder:text-muted-foreground/50"
                                placeholder="44600"
                                type="text"
                    ...
                    <div className="flex items-center gap-3">
                        <input
                            ...
                            type="checkbox"
                        />
                        <label className="text-sm text-muted-foreground font-medium cursor-pointer" htmlFor="default-addr">
