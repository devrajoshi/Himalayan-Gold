...
export default function OrderHistoryPage() {
    const orders = [
        { id: "#HG-98421", date: "Oct 24, 2023", status: "Delivered", total: "$142.00", items: 3 },
        { id: "#HG-97210", date: "Sep 12, 2023", status: "Shipped", total: "$89.00", items: 1 },
        { id: "#HG-96554", date: "Aug 05, 2023", status: "Delivered", total: "$214.50", items: 4 },
        { id: "#HG-95432", date: "Jul 18, 2023", status: "Cancelled", total: "$42.00", items: 1 },
        { id: "#HG-94321", date: "Jun 30, 2023", status: "Delivered", total: "$128.00", items: 2 },
    ];
        ...
        <div className="space-y-12">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-muted-foreground font-medium">Review your past purchases and track current shipments.</p>
                </div>
            ...
            <section className="bg-background rounded-3xl shadow-sm border border-muted overflow-hidden">
                        ...
                        <tbody className="divide-y divide-muted">
                            {orders.map((order) => (
                                <tr key={order.id} className="hover:bg-muted/30 transition-colors cursor-pointer group">
                                    ...
                                    <td className="px-8 py-6 text-muted-foreground">{order.items} {order.items === 1 ? 'Item' : 'Items'}</td>
                ...
                <div className="p-8 border-t border-muted flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <button className="px-4 py-2 border border-muted rounded-lg text-sm disabled:opacity-50" disabled>Previous</button>
                        <button className="px-4 py-2 bg-forest text-primary rounded-lg text-sm font-bold">1</button>
                        <button className="px-4 py-2 border border-muted rounded-lg text-sm hover:bg-muted/50 transition-colors">2</button>
                        <button className="px-4 py-2 border border-muted rounded-lg text-sm hover:bg-muted/50 transition-colors">Next</button>
                    </div>
                </div>
            </section>
        </div>
