...
export default function RegisterPage() {
  const { register: signup, isLoading, error, isAuthenticated, clearError } = useAuth();
  ...
  return (
    <AuthShell
      ...
      brandTagline="Join a community that values purity, tradition, and the extraordinary gifts of nature."
      brandFooter="Nature's Most Precious Gift"
    >
      <div className="w-full max-w-md animate-fade-in">
        <div className="bg-neutral-900/80 backdrop-blur-xl p-10 sm:p-12 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.3)] border border-white/10">
          ...
          <header className="text-center mb-10">
            <h2 className="font-display text-white text-4xl mb-3">Create Account</h2>
            <p className="text-white/60 font-medium text-sm leading-relaxed">
              Start your journey to the purest honey on earth.
            </p>
          </header>

          {/* Global Error */}
          {error && (
            <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-500 text-xs text-center font-bold tracking-wider uppercase">
              {error}
            </div>
          )}
          ...
          <form className="space-y-7" onSubmit={handleSubmit(onSubmit)}>
            <div className="space-y-2">
              ...
              <input
                id="register-name"
                className={`w-full px-0 py-3 bg-transparent border-0 border-b ${errors.name ? "border-red-500" : "border-white/20"} focus:ring-0 input-underline-glow transition-all duration-300 text-white placeholder:text-white/30 text-sm`}
