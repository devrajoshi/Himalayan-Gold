  ...
  return (
    <div className={cn("flex flex-col items-center select-none", className)}>
      {rows.map((count, rowIndex) => {
        ...
        return (
          <div key={rowIndex} className={cn("flex", rowIndex !== rows.length - 1 ? "-mb-11" : "")}>
            {rowImages.map((img, i) => (
    ...
    <section className="py-32 bg-white/40 backdrop-blur-md relative overflow-hidden">
      <div className="container mx-auto px-6">
        <div className="text-center mb-20">
          <h2 className="text-5xl md:text-6xl font-display font-bold text-forest mb-6 italic">
            {title}
          </h2>
          <p className="text-forest/70 max-w-2xl mx-auto italic text-xl">{description}</p>
        </div>
